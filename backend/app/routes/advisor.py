...
def _build_prompt(req: AdvisorRequest, cliff_data: dict) -> str:
    ...
    cliff_lines = "\n".join(
        f"  - ${c['income_level']:,.0f}: {c['description']}"
    ...
    if req.cliff_probability is not None:
        mc_line = (
            f"\n- Monte Carlo cliff probability (income volatility risk): "
            f"{req.cliff_probability * 100:.0f}%"
        )

    user_q = f"\n\nUser's specific question: {req.user_question}" if req.user_question else ""

    return f"""You are CliffSafe, an empathetic benefits cliff advisor helping low-income workers \
understand the hidden cost of earning more. You have access to the user's exact numbers. \
Give specific, actionable advice. Never be generic — always reference their actual dollar amounts.

USER PROFILE:
- Gross income: ${gross:,.0f}/yr
- Household size: {hs}
- State: {state.upper()}
...
- Gross income: ${gross:,.0f}/yr
- Net take-home (after taxes): ${net:,.0f}/yr
- SNAP benefit: ${benefits['snap']:,.0f}/yr
- Medicaid value: ${benefits['medicaid']:,.0f}/yr
- Housing assistance: ${benefits['housing']:,.0f}/yr
- Childcare subsidy: ${benefits['childcare']:,.0f}/yr
- Total benefits package: ${benefits['total']:,.0f}/yr
- TRUE total compensation (take-home + benefits): ${(net + benefits['total']):,.0f}/yr
- Effective marginal rate (taxes + benefit loss): {emr * 100:.0f}%
  (Every $1 raise only nets {max(0, (1-emr)*100):.0f}¢ in real purchasing power){mc_line}

CLIFF POINTS (income levels where net compensation drops):
{cliff_lines}
...
2. Use their specific dollar amounts throughout — never be vague.
3. Explain what happens if they get a raise or work more hours (use actual numbers).
4. Give 2-3 concrete, actionable strategies to protect their benefits or maximize real net income.
5. Be empathetic but direct. This is real money that affects their family.
6. Keep the response to 3-4 paragraphs — clear, scannable, no jargon.{user_q}"""
