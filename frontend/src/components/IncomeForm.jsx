  ...
  return (
    ...
    >
      <div style={{ marginBottom: "2rem" }}>
        ...
        <div style={{ width: "36px", height: "2px", background: "#c0392b", margin: "0 auto" }} />
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.4rem" }}>
        <Field label="Annual Gross Income ($)">
          <input
            ...
            required min={0}
            disabled={loading}
            style={{ ...fieldStyle, ...focusStyle("gross_income"), opacity: loading ? 0.6 : 1 }}
          />
        </Field>
        ...
        <Field label="Household Size">
          <select
            ...
            onBlur={() => setFocused(null)}
            disabled={loading}
            style={{ ...fieldStyle, ...focusStyle("household_size"), cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.6 : 1 }}
          >
            {[1,2,3,4,5,6].map((n) => (
        ...
        <Field label="State">
          <select
            ...
            onBlur={() => setFocused(null)}
            disabled={loading}
            style={{ ...fieldStyle, ...focusStyle("state"), cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.6 : 1 }}
          >
        ...
        <Field label="Employment Type">
          <select
            ...
            onBlur={() => setFocused(null)}
            disabled={loading}
            style={{ ...fieldStyle, ...focusStyle("employment_type"), cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.6 : 1 }}
          >
        ...
        <button
          ...
          style={{
            ...
            padding: "0.95rem", fontSize: "1rem", fontWeight: 600,
          ...
          onMouseEnter={(e) => { if (!loading) { e.currentTarget.style.background = "#a93226"; e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(192,57,43,0.4)"; } }}
          onMouseLeave={(e) => { if (!loading) { e.currentTarget.style.background = "#c0392b"; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 16px rgba(192,57,43,0.3)"; } }}
        >
          {loading ? (
            <>
              <ButtonSpinner />
        ...
        <p style={{ textAlign: "center", fontSize: "0.75rem", color: "#aaa", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
